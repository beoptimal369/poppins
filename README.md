...
- **Single Binary Deployment:**
    - Python applications require a Python interpreter and installed dependencies to run. Deploying to production often involves virtual environments, `Docker` containers, or managing system packages.
...
- IF Rust is NOT installed THEN install Rust: https://rust-lang.org/learn/get-started/
- IF Rust is already installed THEN update Rust `rustup update`
- Create a Project: `cargo new example`
- Install Poppins: `cargo install poppins`
- Create a `train.xml`: `poppins bootstrap`
- Update `train.xml` w/ the data you'd love your AI model to be an expert on
- **(not yet implemented)** Create an AI model: `poppins train`
- **(not yet implemented)** Ask AI model questions: `poppins infer`
